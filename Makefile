...
converter:
	mkdir -p build/host
	$(CC) -std=c99 -O2 -Wall -Wextra -Werror -Isrc/core tools/pt24g_convert.c src/platform/mod_import.c src/platform/project_import.c src/platform/pp20_import.c src/platform/project_file.c src/platform/mod_file.c src/platform/file_save.c src/core/document.c src/core/pp20.c src/core/safe_save.c src/core/mod_project.c src/core/mod_inspect.c src/core/project.c src/core/channels.c src/core/pcm.c -o build/host/PT24GConvert
