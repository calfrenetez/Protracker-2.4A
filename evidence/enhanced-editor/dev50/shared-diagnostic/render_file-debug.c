...
static int begin(struct file *f)
{
    ...
    for(i=0;i<32;++i) {
        ...
        printf("WAVSTAGE create %s\n",f->directory);fflush(stdout);
#endif
        f->owned=1;snprintf(f->temporary,sizeof(f->temporary),"%s/data",f->directory);
...
#ifdef PT_TRACE_STAGING
        {BPTR lock=Lock((STRPTR)f->directory,ACCESS_READ);printf("WAVSTAGE removed %s lock=%lu error=%ld\n",f->directory,(unsigned long)lock,(long)IoErr());if(lock)UnLock(lock);fflush(stdout);}
#endif
#endif
        f->owned=0;
    }
