...
#endif
#include "stem_file.h"
#ifdef PT_TRACE_STAGING
static char last_deleted[1300];
#endif
static int directory_new(const char *path)
{
    ...
    BPTR lock;
#ifdef PT_TRACE_STAGING
    if(last_deleted[0]) {lock=Lock((STRPTR)last_deleted,ACCESS_READ);printf("STAGE previous %s lock=%lu error=%ld\n",last_deleted,(unsigned long)lock,(long)IoErr());if(lock)UnLock(lock);fflush(stdout);}
#endif
    lock=CreateDir((STRPTR)path);
    if(!lock)return IoErr()==ERROR_OBJECT_EXISTS?0:-1;
    UnLock(lock);
#ifdef PT_TRACE_STAGING
    printf("STAGE create %s\n",path);fflush(stdout);
#endif
    return 1;
#else
...
#ifdef PT_TRACE_STAGING
    strcpy(last_deleted,path);
    {BPTR lock=Lock((STRPTR)path,ACCESS_READ);printf("STAGE removed %s lock=%lu error=%ld\n",path,(unsigned long)lock,(long)IoErr());if(lock)UnLock(lock);fflush(stdout);}
#endif
#else
