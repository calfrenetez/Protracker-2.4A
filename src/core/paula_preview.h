...
#include "pcm.h"
/* Mono/stereo Paula preview preparation. Storage is caller-owned Fast workspace; padding
                                                         ...
                                                         uint32_t rate,uint32_t *frames)
{
    ...
    if(!frames)return PT_PCM_INVALID;
                                                         ...
                                                         struct pt_pcm *out)
{
    ...
    if(out->capacity<(size_t)padded*source->channels)return PT_PCM_CAPACITY;
    if(out->channels!=source->channels || out->bits!=8 || out->frames!=padded)return PT_PCM_INVALID;
    src_bytes=(size_t)source->frames*source->channels*sizeof(int32_t);dst_bytes=(size_t)padded*source->channels*sizeof(int32_t);
