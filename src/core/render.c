        ...
        if(r->timeline.flow.fresh)for(ch=0;ch<r->view.channels.count;++ch)if(r->tracks&(1U<<ch)) {
            ...
            if(e->kind==PT_NOTE_NONE && e->instrument && v->sounding) {
                ...
                if(e->instrument!=v->instrument) {
                    ...
                    if((e->effect && e->effect!=1 && e->effect!=2 && e->effect!=3 && e->effect!=5 && e->effect!=4 && e->effect!=6 && e->effect!=7 && e->effect!=12 && e->effect!=10 && !(e->effect==14 && ((e->parameter>>4)==1 || (e->parameter>>4)==2 || (e->parameter>>4)==13 || (e->parameter>>4)==10 || (e->parameter>>4)==11 || (e->parameter>>4)==12))) || (r->offset_tracks&(1U<<ch)) || !handoff_sample(a) || (!handoff_sample(b) && !silent_handoff_sample(b)) ||
