...
void pt_editor_draw(const struct pt_editor *e,struct pt_canvas *c,const uint8_t font[580])
{
    ...
    if(e->panel>=5) {
        ...
        static const char *basic[4][3]={{"PREV POS","PAT -","PAT +"},{"NEXT POS","ADD POS","NEW PAT"},{"UNDO","REDO","MORE"},{"BACK","PLAY","STOP"}};
        static const char *tools[4][3]={{"PREV POS","INSERT","REMOVE"},{"NEXT POS","MOVE UP","MOVE DN"},{"UNDO","REDO","BASIC"},{"BACK","PLAY","STOP"}};
        ...
        for(r=0;r<4;++r)for(i=0;i<3;++i)label(c,font,230+(int)i*123,21+(int)r*19,123,19,e->song_tools?tools[r][i]:basic[r][i],0);
        ...
        label(c,font,353,21,123,19,s,e->number_field==9);label(c,font,476,21,123,19,"ALL / ONE",0);
        ...
        label(c,font,230,59,369,19,"RENDER NEW WAV",0);
