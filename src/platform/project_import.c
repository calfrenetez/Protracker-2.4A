...
{
    struct input *in=context;size_t have=0,count;
    ...
    count=in->length-pos;if(count>sizeof(in->block))count=sizeof(in->block);
    if(lseek(in->fd,(off_t)pos,SEEK_SET)!=(off_t)pos)return 0;
    in->used=0;
    while(have<count) {long got=read_retry(in->fd,in->block+have,count-have);if(got<=0)return 0;have+=(size_t)got;}
    in->offset=pos;in->used=count;memcpy(out,in->block,n);return 1;
}
...
int pt_project_file_candidate(const char *path)
{
    struct input in={0};uint8_t tag[8];int ok;
    if(!path)return 0;
    in.fd=open(path,O_RDONLY);if(in.fd<0)return 0;in.length=8;
    ok=read_at(&in,0,tag,8) && !memcmp(tag,"PT24G\r\n\032",8);
    if(close(in.fd))return 0;
    return ok;
}
enum pt_project_result pt_project_file_load(struct pt_document *d,const char *path,size_t limit,size_t budget)
{
    ...
    in.length=(size_t)end;r=pt_document_load_project_reader(d,read_at,&in,in.length,budget,finish);
done:
