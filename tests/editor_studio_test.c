...
int main(void)
{
    ...
    int32_t pcm[4]={257,-513,1025,-2049};unsigned baseline,done;const struct pt_pcm *out;
    ...
    start(&owner,&options);pt_editor_dispose(e);assert(!owner.song && !e->sampler.bytes);
    ...
    puts("EDITOR STUDIO PASS: real pinned song stopped by edit/undo/dispose, navigation preserved, guard ownership and cleanup");return 0;
}
