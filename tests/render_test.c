...
int main(void)
{
    ...
    memcpy(events+64*4,events,64*4*sizeof(*events));events[0].effect=14;events[0].parameter=0xf1;o.pattern_only=1;o.pattern=1;reset_capture(&c,pcm);
    ...
    assert(pt_render_stream(&p,&o,receive,&c,NULL,NULL,&report)==PT_RENDER_EFFECT && !c.calls && !memcmp(&report,&before,sizeof(report)));events[0].effect=0;events[0].parameter=0;
    sample.finetune=1;
