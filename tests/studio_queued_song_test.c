...
int main(void)
{
    ...
    for(mode=0;mode<3;++mode)for(partition=PT_SONG_FIRST_BLOCK;partition<=256;partition=partition==1?17:partition==17?256:257) {
        ...
        {struct pt_studio_queue *queue=pt_studio_queue_open(&a,2);struct pt_studio_pump pump;
            ...
            while(!done) {
                unsigned i;uint64_t ticket;enum pt_queue_result qr;
                for(i=0;i<7;++i)assert(pt_studio_pump_step(&pump,partition)!=PT_PUMP_ERROR);
                qr=pt_studio_queue_acquire(queue,&block,&ticket);
                if(qr==PT_QUEUE_OK) {
                    assert(emitted+block->frames*2<=used);assert(!memcmp(block->data,reference+emitted,block->frames*2*sizeof(int32_t)));emitted+=block->frames*2;
                    assert(pt_studio_queue_release(queue,ticket)==PT_QUEUE_OK);
                } else {assert(qr==PT_QUEUE_EMPTY || qr==PT_QUEUE_DONE);done=qr==PT_QUEUE_DONE;}
            }
    ...
    puts("QUEUED SONG PASS: reference audio, tempo/delay, pre-roll, partition invariance, protocol and allocation refusal");return 0;
}
