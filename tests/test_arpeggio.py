...
class Arpeggio(unittest.TestCase):
    def test_native_stored_output_periods_and_pcm(self):
        evidence=ROOT/'evidence/enhanced-editor/dev37/native'
        report=json.loads((evidence/'native-arp.json').read_text())
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            bounds=Path(tmp)/'bounds'
            subprocess.run([*flags,'tests/arp_test.c',*COMMON,'-o',str(bounds)],cwd=ROOT,check=True)
            subprocess.run([str(bounds)],check=True)
            ...
            print("ARPEGGIO: nine repeated native traces match host/m68k period words and ramp PCM, including zero-period refusal")
