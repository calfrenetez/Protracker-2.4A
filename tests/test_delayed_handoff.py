...
from make_handoff_fixtures import delayed_handoff_fixtures
from test_instrument_emulator import decode_trace
class DelayedHandoff(unittest.TestCase):
    def test_pinned_volume_and_pcm(self):
        evidence=ROOT/'evidence/enhanced-editor/dev84/native-reference'
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for name,data,meta in delayed_handoff_fixtures():
                ...
                for offset in range(0,len(trace),140):
                    ...
                    self.assertEqual(int.from_bytes(r[72:74],'big'),2 if row==1 and r[10]>=meta['delay'] else 1)
