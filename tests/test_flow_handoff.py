...
from make_handoff_fixtures import flow_fixtures
from test_instrument_emulator import decode_trace
class FlowHandoff(unittest.TestCase):
    def test_pinned_volume_and_pcm(self):
        evidence=ROOT/'evidence/enhanced-editor/dev79/native-reference'
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for name,data,meta in flow_fixtures():
                ...
                self.assertEqual(seen,{0,1} if meta['control']==0xe1 else {0,1,2})
                self.assertEqual(row_counts[1],12)
