...
from make_handoff_fixtures import modulation_fixtures
from test_instrument_emulator import decode_trace
class ModulationHandoff(unittest.TestCase):
    def test_pinned_volume_and_pcm(self):
        evidence=ROOT/'evidence/enhanced-editor/dev74/native-reference'
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for name,data,meta in modulation_fixtures():
                ...
                seen=set();periods=set();volumes=set()
                for offset in range(0,len(trace),140):
                    ...
                    counter=r[10]
                    if row==1:
                        periods.add(int.from_bytes(r[44:46],'big'));volumes.add(r[32])
                    if meta['effect'] in (0,4):self.assertEqual(r[32],64 if row==0 else 32)
                    if meta['effect']==6:self.assertEqual(r[32],64 if row==0 else 32-counter)
                    self.assertEqual(int.from_bytes(r[72:74],'big'),1)
                self.assertEqual(seen,{0,1})
                if meta['effect']==7:self.assertGreater(len(volumes),1)
                else:self.assertGreater(len(periods),1)
