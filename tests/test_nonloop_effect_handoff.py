...
from make_handoff_fixtures import nonloop_effect_fixtures
from test_instrument_emulator import decode_trace
class NonloopEffectHandoff(unittest.TestCase):
    def test_pinned_volume_and_pcm(self):
        evidence=ROOT/'evidence/enhanced-editor/dev88/native-reference'
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for name,data,meta in nonloop_effect_fixtures():
                ...
                for offset in range(0,len(trace),140):
                    ...
                    self.assertEqual(r[32],32 if row==1 else 64)
                    if row==1 and meta['kind']=='offset':
                        self.assertEqual(int.from_bytes(r[52:56],'big'),4412)
                        self.assertEqual(int.from_bytes(r[56:58],'big'),32)
                    self.assertEqual(int.from_bytes(r[72:74],'big'),2+r[10]//2 if row==1 and meta['kind']=='retrigger' else 1)
                self.assertEqual(seen,{0,1,2} if meta['kind']=='return' else {0,1})
