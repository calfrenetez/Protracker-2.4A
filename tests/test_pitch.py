...
from test_pitch_emulator import decode_trace
...
class Pitch(unittest.TestCase):
    def test_native_stored_output_periods_and_pcm(self):
        evidence=ROOT/'evidence/enhanced-editor/dev35/native'
        report=json.loads((evidence/'native-pitch.json').read_text())
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            subprocess.run([*flags,'tests/render_pitch_test.c','src/core/render.c','src/core/timeline.c','src/core/frame_clock.c','src/core/voice.c',*COMMON,'-o',str(pcm)],cwd=ROOT,check=True)
            ...
            print('PITCH: seven repeated native traces match host/m68k state, PCM and zero-period refusal')
