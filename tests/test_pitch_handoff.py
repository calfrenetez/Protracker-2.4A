...
from make_handoff_fixtures import pitch_fixtures
from test_instrument_emulator import decode_trace
class PitchHandoff(unittest.TestCase):
    def test_pinned_volume_and_pcm(self):
        evidence=ROOT/'evidence/enhanced-editor/dev73/native-reference'
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for name,data,meta in pitch_fixtures():
                ...
                for offset in range(0,len(trace),140):
                    ...
                    delta=0 if row==0 else 3*counter if meta['effect'] in (1,2) else 3
                    up=meta['effect']==1 or meta['parameter']==0x13
                    self.assertEqual(int.from_bytes(r[44:46],'big'),428+(-delta if up else delta))
