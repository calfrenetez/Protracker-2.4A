...
class RenderInvertFile(unittest.TestCase):
    def test_verified_private_effect_file(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            subprocess.run(['cc','-std=c99','-O1','-g','-Wall','-Wextra','-Werror','-fsanitize=address,undefined','-Isrc/core','-Isrc/platform','tests/render_invert_file_test.c','src/platform/render_file.c','src/platform/render_invert_file.c',*[f'src/core/{s}.c' for s in sources],'-o',str(exe)],cwd=ROOT,check=True)
