...
from make_handoff_fixtures import slide_fixtures
from test_instrument_emulator import decode_trace
class SlideHandoff(unittest.TestCase):
    def test_pinned_volume_and_pcm(self):
        evidence=ROOT/'evidence/enhanced-editor/dev70/native-reference'
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for name,data,meta in slide_fixtures():
                ...
                for offset in range(0,len(trace),140):
                    ...
                    counter=r[10]
                    if row==0:volume=64
                    elif meta['effect']==10:volume=32+(counter if meta['parameter']==16 else -counter)
                    else:volume=33 if meta['parameter']==0xa1 else 31
