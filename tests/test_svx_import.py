...
class SvxImport(unittest.TestCase):
            ...
            for faults in (False,True):
                subprocess.run(['cc','-std=c99','-O1','-g','-Wall','-Wextra','-Werror','-fsanitize=address,undefined','-Isrc/core',
                    *(['-Dread=pt_test_read','tests/render_read_faults.c'] if faults else []),
                    'tests/svx_import_test.c','src/platform/raw_import.c',*SOURCES[1:],'-o',str(exe)],cwd=ROOT,check=True)
                subprocess.run([str(exe),str(Path(tmp)/'sample.iff')],check=True)
