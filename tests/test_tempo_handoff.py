...
from make_handoff_fixtures import tempo_fixtures
from test_instrument_emulator import decode_trace
class TempoHandoff(unittest.TestCase):
    def test_pinned_volume_and_pcm(self):
        evidence=ROOT/'evidence/enhanced-editor/dev80/native-reference'
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for name,data,meta in tempo_fixtures():
                ...
                for offset in range(0,len(trace),140):
                    ...
                    self.assertEqual(int.from_bytes(r[12:14],'big'),137 if row==1 and meta['value']==137 else 125)
                    self.assertEqual(r[32],64 if row==0 else 32)
                    self.assertEqual(int.from_bytes(r[72:74],'big'),1)
                self.assertEqual(seen,{0,1})
                self.assertEqual(row_counts[1],3 if meta['value']==3 else 6)
