...
from make_handoff_fixtures import tone_note_fixtures
from test_instrument_emulator import decode_trace
class ToneNoteHandoff(unittest.TestCase):
    def test_pinned_volume_and_pcm(self):
        evidence=ROOT/'evidence/enhanced-editor/dev76/native-reference'
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for name,data,meta in tone_note_fixtures():
                ...
                for offset in range(0,len(trace),140):
                    ...
                    counter=r[10]
                    if row==2:periods.add(int.from_bytes(r[44:46],'big'))
                    self.assertEqual(r[32],64 if row<2 else 32-counter if meta['effect']==5 else 32)
