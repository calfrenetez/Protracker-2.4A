...
class Vibrato(unittest.TestCase):
    def test_native_stored_output_periods_and_pcm(self):
        evidence=ROOT/'evidence/enhanced-editor/dev38/native'
        report=json.loads((evidence/'native-vib.json').read_text())
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            subprocess.run([*flags,'tests/vib_test.c',*COMMON,'-o',str(bounds)],cwd=ROOT,check=True)
            ...
            print("VIBRATO: eleven repeated native traces match host/m68k period words and ramp PCM")
