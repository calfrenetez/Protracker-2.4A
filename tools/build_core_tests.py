...
def main():
    ...
    inputs['PT24GEdit'] += ['src/platform/sample_svx_file.c']
    ...
    inputs['PTModStreamTest'] = ['tests/mod_stream_test.c', 'src/platform/mod_file.c', 'src/platform/file_save.c', 'src/core/safe_save.c', 'src/core/mod_project.c', 'src/core/mod_inspect.c', 'src/core/project.c', 'src/core/channels.c', 'src/core/pcm.c']
    inputs['PTExecModStreamTest'] = ['tests/native_exec_mod_stream_test.c', *inputs['PTModStreamTest'][1:]]
    inputs['PT24GEdit'] += ['src/platform/raw_import.c']
    inputs['PTRawImportTest'] = ['tests/raw_import_test.c', 'src/platform/raw_import.c', *inputs['PTSamplerTest'][1:]]
    ...
    inputs['PT24GEdit'] += ['src/platform/sample_import.c']
    inputs['PTSampleDispatchTest'] = ['tests/sample_dispatch_test.c', 'src/platform/sample_import.c', 'src/platform/raw_import.c', 'src/platform/pp20_import.c', *inputs['PTSourceTest'][1:]]
    inputs['PTExecSampleDispatchTest'] = ['tests/native_exec_sample_dispatch_test.c', *inputs['PTSampleDispatchTest'][1:]]
    ...
    inputs['PTProjectImportTest'] = ['tests/project_import_test.c','src/platform/project_import.c', *inputs['PTDocumentTest'][1:]]
    for target in ['PT24GEdit','PT24GConvert','PTPp20Test']:inputs[target] += ['src/platform/pp20_import.c']
    ...
    inputs['PT24GRender'] += ['src/platform/render_invert_file.c',*invert_sources]
    ...
    inputs['PTAmiGusRegisterSessionTest'] = ['tests/amigus_register_session_test.c','src/core/amigus_register_port.c','src/core/amigus_session.c','src/core/studio_consumer.c','src/core/studio_queue.c','src/core/amigus_fifo.c','src/core/amigus_pcm_pack.c','src/core/pcm.c']
    inputs['PTExecAmiGusRegisterSessionTest'] = ['tests/native_exec_amigus_register_session_test.c',*inputs['PTAmiGusRegisterSessionTest'][1:]]
    inputs['PTAmiGusRegisterTest'] = ['tests/amigus_register_port_test.c','src/core/amigus_register_port.c']
    ...
    inputs['PTQueuedSongTest'] = ['tests/studio_queued_song_test.c','src/core/studio_consumer.c','src/core/studio_pump.c','src/core/studio_queue.c',*inputs['PTStudioSongTest'][1:]]
    ...
    report = {'compiler': subprocess.check_output([cc, '--version'], text=True).splitlines()[0],
              ...
              'sources': {name: digest(ROOT / name) for name in
                          sorted(set(sum(inputs.values(), [])) | {'src/core/recent.h', 'src/platform/recent_file.h', 'src/native/file_request.h', 'src/core/playback.h', 'src/core/scope.h', 'src/core/flow.h', 'src/core/frame_clock.h', 'src/core/timeline.h', 'src/core/voice.h', 'src/core/render.h', 'src/core/render_commands.h','src/core/render_invert.h','src/core/invert_bank.h', 'src/core/studio_plan.h', 'src/core/studio_song.h', 'src/core/studio_queue.h', 'src/core/studio_pump.h', 'src/editor/sampler_song.h', 'src/editor/editor_studio.h', 'src/core/stems.h', 'src/platform/stem_file.h', 'src/core/pitch.h', 'src/core/pitch_tables.h', 'tools/generate_pitch_tables.py', 'src/platform/render_file.h','src/platform/render_invert_file.h', 'tools/build_core_tests.py', 'src/native/paula.h', 'src/native/paula_memory.h', 'src/core/paula_sync.h', 'src/native/replay_abi.s', 'tools/prepare_replay.py', 'tools/prepare_flow_trace.py', 'tools/prepare_pitch_trace.py', 'tools/prepare_sample_trace.py', 'tools/prepare_invert_trace.py', 'tools/prepare_volume_trace.py', 'vendor/pt23f/replayer/PT2.3F_replay_cia.s', 'src/core/channels.h', 'src/core/pcm.h',
