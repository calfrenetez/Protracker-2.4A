...
def main():
    ...
    run=share/('delaypcm'+str(time.time_ns()));run.mkdir();out=ROOT/'build/dev/delay-render-evidence'/run.name;out.mkdir(parents=True)
    shutil.copyfile(ROOT/'build/dev/PTOffsetRenderTest',run/'PTOffsetRenderTest')
    evidence=ROOT/'evidence/enhanced-editor/dev42/native';commands=[('safety',[])];inputs={}
    for name,data,meta in fixtures():
        ...
        for ch in ([1] if name=='trigger_repeat' else [0]):commands.append((name+'-ch'+str(ch),[name+'.mod',name+'0.log',str(ch)]))
    ...
    try:
        ...
        report={'run_id':run.name,'elapsed_seconds':round(time.monotonic()-start,3),'binary_sha256':digest(run/'PTOffsetRenderTest'),'input_sha256':inputs,'logs':logs,'stopped_audio':audio,'environment':{c:emu.command(c) for c in ['GET_STATUS','GET_VERSION','GET_CPU_MODEL','GET_MEMORY_CONFIG']},'scope':'Reference immutable PCM at 48k/rate428 against dev42 native delayed-trigger ranges; nonloops stop after initial segment. No native sample-word mutation, analogue audio or physical acceptance.'}
        (out/'native-delay-render.json').write_text(json.dumps(report,indent=2)+'\n');print('PASS: '+str(out),flush=True)
    finally:
