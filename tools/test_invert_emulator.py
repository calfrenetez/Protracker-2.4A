#!/usr/bin/env python3
"""Capture EFx byte mutations without changing shipping replay; reserve Amiberry first."""
...
def main():
    ...
    assert digest(ROOT/'build/dev/PTInvertTraceTest')==manifest['binaries']['PTInvertTraceTest']['sha256']
    shutil.copyfile(ROOT/'build/dev/PTInvertTraceTest',run/'PTInvertTraceTest')
    ...
    script+=['PTInvertTraceTest baseline.mod 160 >baseline.log','Echo $RC >baseline.rc']
    for name,data,meta in cases:
        (run/(name+'.mod')).write_bytes(data)
        for repeat in range(2):
            stem=name+str(repeat);script += [f'PTInvertTraceTest {name}.mod {meta["max_ticks"]} >{stem}.log',f'Echo $RC >{stem}.rc']
    ...
    try:
        ...
        assert b''.join(trace[i:i+52] for i in range(0,len(trace),164))==old
        ...
        for name,data,meta in cases:
            ...
            results[name]={**meta,'native_trace':'CAPTURED TWICE - EXACT MATCH','reason':reason,'ticks':len(trace)//164,'fixture_sha256':hashlib.sha256(data).hexdigest(),'trace_sha256':hashlib.sha256(trace).hexdigest()}
        audio=emu.command('GET_AUDIO_STATE');assert all('ch%d_dma=0'%i in audio.split('\t') for i in range(4))
        report={'schema':1,'record_bytes':164,'run_id':run.name,'elapsed_seconds':round(time.monotonic()-start,3),'diagnostic_sha256':digest(run/'PTInvertTraceTest'),'baseline_first52':'EXACT DEV38 MATCH','cases':results,'stopped_audio':audio,'environment':{c:emu.command(c) for c in ['GET_STATUS','GET_VERSION','GET_CPU_MODEL','GET_MEMORY_CONFIG']},'scope':'EFx channel0 cursor, accumulator and first16 loop bytes plus sample/register snapshots. Renderer acceptance requires separate parity checks; no physical acceptance.'}
        (out/'native-invert.json').write_text(json.dumps(report,indent=2)+'\n');print('PASS: '+str(out),flush=True)
    finally:
