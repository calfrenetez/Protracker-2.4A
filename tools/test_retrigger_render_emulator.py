...
def main():
    ...
    run=share/('retrigpcm'+str(time.time_ns()));run.mkdir();out=ROOT/'build/dev/retrigger-render-evidence'/run.name;out.mkdir(parents=True)
    ...
    for name,data,meta in fixtures():
        ...
        for ch in range(1):commands.append((name+'-ch'+str(ch),[name+'.mod',name+'0.log',str(ch)]))
    ...
    try:
        ...
        report={'run_id':run.name,'elapsed_seconds':round(time.monotonic()-start,3),'binary_sha256':digest(run/'PTOffsetRenderTest'),'input_sha256':inputs,'logs':logs,'stopped_audio':audio,'environment':{c:emu.command(c) for c in ['GET_STATUS','GET_VERSION','GET_CPU_MODEL','GET_MEMORY_CONFIG']},'scope':'Reference immutable PCM at 48k/rate428 against dev42 native retrigger ranges; nonloops stop after initial segment. No native sample-word mutation, analogue audio or physical acceptance.'}
        (out/'native-retrigger-render.json').write_text(json.dumps(report,indent=2)+'\n');print('PASS: '+str(out),flush=True)
    finally:
