#!/usr/bin/env python3
"""Native sample attributes, shared PCM history and exact MOD export. Reserve Amiberry first."""
import json
...
def main():
    ...
    run=share/('attributes'+str(time.time_ns()));run.mkdir();out=ROOT/'build/dev/attributes-evidence';out.mkdir(exist_ok=True)
    for name in ['PT24GEdit','PTSampleAttributesTest']:shutil.copyfile(ROOT/'build/dev'/name,run/name)
    shutil.copyfile(ROOT/'evidence/baseline/mod.baseline',run/'input.mod')
    baseline=(run/'input.mod').read_bytes();target_volume=max(0,baseline[45]-2)
    ...
    try:
        launch.write_text('\n'.join(['FailAt 21','Wait 5','Stack 65536','CD PTDEV:'+run.name,
            'PTSampleAttributesTest native >attributes.log','Echo $RC >attributes.rc',
        ...
        frame('status=READY -');assert (run/'attributes.rc').read_text().strip()=='0'
        key(0x57);frame('PLAYING SONG - PAULA CIA')
        key(0x4f,alt=True);frame('revision=1 dirty=1 status=SAMPLE UPDATED');audio_off()
        key(0x4f,alt=True);key(0x4c,alt=True);frame('revision=3 dirty=1 status=SAMPLE UPDATED')
        key(0x36,True,True);key(0x35);key(0x20);key(0x21);key(0x21);key(0x44);frame('revision=4 dirty=1 status=SAMPLE UPDATED')
        capture('01-sample-name-volume-finetune.png')
        key(0x28,True);key(0x17);frame('revision=5 dirty=1 status=SAMPLE UPDATED')
        key(0x36,True,True);key(0x13);key(0x12);key(0x34);key(0x44);frame('revision=6 dirty=1 status=SAMPLE UPDATED')
        key(0x31,True);key(0x31,True);frame('revision=4 dirty=1 status=UNDO')
        key(0x36,True,True);key(0x32);key(0x45);frame('revision=4 dirty=1 status=NAME ENTRY CANCELLED')
        key(0x31,True,True);frame('revision=5 dirty=1 status=REDO');key(0x31,True);frame('revision=4 dirty=1 status=UNDO')
        capture('02-shared-pcm-restored.png');key(0x21,True);frame('dirty=0 status=PROJECT SAVED')
        saved=(run/'saved.ptg').read_bytes();record=samples(saved)
        assert record[:32]==b'BASS'+bytes(28) and record[42:44]==bytes([target_volume,1])
        expected=bytearray(baseline);expected[20:42]=b'BASS'+bytes(18);expected[44]=1;expected[45]=target_volume
        request('mod');filename('exact.mod');key(0x44);frame('MOD EXPORTED AND VERIFIED')
        assert (run/'exact.mod').read_bytes()==expected,'Only sample name, volume and finetune should differ'
        ...
        capture('03-reopened-sample-attributes.png');key(0x45);wait(lambda:(run/'done').exists())
        ...
        report={'run_id':run.name,'elapsed_seconds':round(time.monotonic()-start,3),
            'binaries':{n:digest(run/n) for n in ['PT24GEdit','PTSampleAttributesTest']},
            'native_allocation_failure_and_200_metadata_edits':True,'name_volume_finetune_controls':True,
            'sample_edit_stops_old_audio_snapshot':True,'mixed_pcm_and_metadata_undo':True,'cancel_preserves_redo':True,
            'exact_mod_only_attribute_changes':True,'exact_project_crc_and_reopen_identity':True,'normal_exits':2,'stopped_audio':audio,
            'logs':{n:(run/n).read_text() for n in ['attributes.log','editor.log','reopened.log']},
            'environment':{c:emu.command(c) for c in ['GET_STATUS','GET_VERSION','GET_CPU_MODEL','GET_MEMORY_CONFIG']}}
        (out/'native-attributes.json').write_text(json.dumps(report,indent=2)+'\n')
        for n in ['saved.ptg','reopened.ptg','exact.mod']:shutil.copyfile(run/n,out/n)
        print('PASS: native sample name/volume/finetune, shared PCM ownership, mixed undo, exact MOD export and PTG reopen')
    finally:
        ...
        print('Sample attributes test released:',run)
