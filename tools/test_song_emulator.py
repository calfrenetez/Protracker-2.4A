#!/usr/bin/env python3
"""Native song construction, undo, strict export and replay restart. Reserve Amiberry first."""
import json
...
def main():
    ...
    run=share/('song'+str(time.time_ns()));run.mkdir();out=ROOT/'build/dev/song-evidence';out.mkdir(exist_ok=True)
    ...
    try:
        ...
        expected[950]=3;expected[952:955]=bytes([0,1,0]);expected[2108:2112]=bytes([1,172,16,0])
        ...
        capture('01-song-construction.png')
        offset=key(0x57);wait(lambda:any('EDITOR REPLAY active=1' in row and 'order=1 pattern=1' in row and 'period=428,' in row for row in log()[offset:].splitlines()))
        ...
        key(0x19,True);capture('02-reopened-song.png');key(0x45);key(0x45);wait(lambda:(run/'done').exists())
        ...
        report={'run_id':run.name,'elapsed_seconds':round(time.monotonic()-start,3),
            ...
            'mixed_song_and_note_undo':True,'exact_mod_orders_pattern_and_note_change':True,
        ...
        (out/'native-song.json').write_text(json.dumps(report,indent=2)+'\n')
        for n in ['saved.ptg','reopened.ptg','exact.mod']:shutil.copyfile(run/n,out/n)
        print('PASS: native song construction, shared undo, replay restart, exact MOD export and PTG reopen')
    finally:
        ...
        print('Song construction test released:',run)
