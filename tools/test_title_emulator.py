#!/usr/bin/env python3
"""Native song-title editing, undo and replay isolation. Reserve Amiberry first."""
import json
...
def main():
    ...
    run=share/('titles'+str(time.time_ns()));run.mkdir();out=ROOT/'build/dev/title-evidence';out.mkdir(exist_ok=True)
    for name in ['PT24GEdit','PTPatternTest','PTPaulaTest']:shutil.copyfile(ROOT/'build/dev'/name,run/name)
    ...
    try:
        ...
        letter_keys=dict(zip('ABCDEFGHIJKLMNOPQRSTUVWXYZ',[0x20,0x35,0x33,0x22,0x12,0x23,0x24,0x25,0x17,0x26,0x27,0x28,0x37,0x36,0x18,0x19,0x10,0x13,0x21,0x14,0x16,0x34,0x11,0x32,0x15,0x31]));letter_keys[' ']=0x40
        def title(text):
            key(0x14,True,True)
            for char in text:key(letter_keys[char])
            return key(0x44)
        def active(after):wait(lambda:'EDITOR REPLAY active=1' in log()[after:])
        key(0x57);frame('PLAYING SONG - PAULA CIA')
        offset=title('ENHANCED PROTRACKER TITLE');frame('revision=1 dirty=1 status=SONG TITLE UPDATED');active(offset)
        capture('01-long-title-playing.png');key(0x37,True,True);frame('MOD EXPORT REFUSED')
        key(0x31,True);frame('revision=0 dirty=0 status=UNDO')
        key(0x14,True,True);key(0x32);key(0x45);frame('revision=0 dirty=0 status=NAME ENTRY CANCELLED')
        key(0x31,True,True);frame('revision=1 dirty=1 status=REDO');key(0x31,True)
        key(0x28,True);key(0x17);frame('revision=2 dirty=1 status=SAMPLE UPDATED');audio_off()
        title('NEW SONG');frame('revision=3 dirty=1 status=SONG TITLE UPDATED')
        key(0x31,True);key(0x31,True);frame('revision=0 dirty=0 status=UNDO')
        title('NEW SONG');frame('revision=4 dirty=1 status=SONG TITLE UPDATED');key(0x21,True);frame('dirty=0 status=PROJECT SAVED')
        ...
        expected=bytearray(baseline);expected[:20]=b'NEW SONG'+bytes(12)
        request('mod');filename('exact.mod');key(0x44);frame('MOD EXPORTED AND VERIFIED')
        assert (run/'exact.mod').read_bytes()==expected,'Only the song title should differ'
        ...
        capture('02-reopened-title.png');key(0x45);wait(lambda:(run/'done').exists())
        ...
        report={'run_id':run.name,'elapsed_seconds':round(time.monotonic()-start,3),
            'binaries':{n:digest(run/n) for n in ['PT24GEdit','PTPatternTest','PTPaulaTest']},
            'native_title_history_tests':True,'long_title_playback_preserves_source':True,'strict_mod_refuses_long_title':True,
            'cancel_preserves_redo':True,'mixed_title_and_pcm_undo':True,'exact_mod_only_title_change':True,
        ...
        (out/'native-title.json').write_text(json.dumps(report,indent=2)+'\n')
        for n in ['saved.ptg','reopened.ptg','exact.mod']:shutil.copyfile(run/n,out/n)
        print('PASS: native song titles, replay isolation, mixed undo, exact MOD export and PTG reopen')
    finally:
        ...
        print('Song title test released:',run)
