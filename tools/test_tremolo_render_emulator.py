...
def main():
    ...
    run=share/('tremolopcm'+str(time.time_ns()));run.mkdir();out=ROOT/'build/dev/tremolo-render-evidence'/run.name;out.mkdir(parents=True)
    shutil.copyfile(ROOT/'build/dev/PTTremoloRenderTest',run/'PTTremoloRenderTest')
    evidence=ROOT/'evidence/enhanced-editor/dev45/native';commands=[];inputs={}
    for name,data,meta in fixtures():
        ...
        for ch in range(4 if name=='trem_waves' else 1):commands.append((name+'-ch'+str(ch),[name+'.mod',name+'0.log',str(ch)]))
    assert {name+'.log' for name,args in commands}.isdisjoint(inputs)
    with tempfile.TemporaryDirectory() as tmp:
        host=Path(tmp)/'offset'
        sources=['tests/render_tremolo_test.c',*['src/core/'+n+'.c' for n in ['render','pitch','timeline','frame_clock','flow','voice','document','pp20','mod_project','mod_inspect','project','channels','pcm']]]
    ...
    for name,args in commands:script+=['PTTremoloRenderTest '+ ' '.join(args)+' >'+name+'.log','Echo $RC >'+name+'.rc']
    ...
    try:
        ...
        report={'run_id':run.name,'elapsed_seconds':round(time.monotonic()-start,3),'binary_sha256':digest(run/'PTTremoloRenderTest'),'input_sha256':inputs,'logs':logs,'stopped_audio':audio,'environment':{c:emu.command(c) for c in ['GET_STATUS','GET_VERSION','GET_CPU_MODEL','GET_MEMORY_CONFIG']},'scope':'Reference immutable PCM at 48k/rate428 against dev45 native volume/pitch events; source ramp extended into a full-sample reference loop. No native sample-word mutation, analogue audio or physical acceptance.'}
        (out/'native-tremolo-render.json').write_text(json.dumps(report,indent=2)+'\n');print('PASS: '+str(out),flush=True)
    finally:
