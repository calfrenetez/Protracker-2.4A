...
def main():
    ...
    run=share/('trigger'+str(time.time_ns()));run.mkdir();out=ROOT/'build/dev/trigger-evidence'/run.name;out.mkdir(parents=True)
    ...
    try:
        ...
        report={'schema':1,'record_bytes':140,'run_id':run.name,'elapsed_seconds':round(time.monotonic()-start,3),'diagnostic_sha256':digest(run/'PTSampleTraceTest'),'baseline_first52':'EXACT DEV38 MATCH','cases':results,'stopped_audio':audio,'environment':{c:emu.command(c) for c in ['GET_STATUS','GET_VERSION','GET_CPU_MODEL','GET_MEMORY_CONFIG']},'scope':'E9x/EDx trigger timing and saved sample ranges only. Renderer enablement and physical acceptance remain open.'}
        (out/'native-trigger.json').write_text(json.dumps(report,indent=2)+'\n');print('PASS: '+str(out),flush=True)
    finally:
        ...
        print('Trigger capture released:',run,flush=True)
